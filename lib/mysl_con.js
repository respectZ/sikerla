...
const mysql_con = {
  ...
  getBahanBaku: (date, callback) => {
    con.query(
      `SELECT * FROM bahan_baku WHERE DATE(waktu) = DATE(?)`,
      [date],
      (err, res) => {
        if (err) callback(err, null);
        else callback(null, res);
  ...
  getProduk: ({ date = null, callback }) => {
    if (date)
      con.query(
        `SELECT * FROM produk WHERE DATE(waktu) = DATE(?)`,
        [date],
        (err, res) => {
          if (err) callback(err, null);
          else callback(null, res);
      ...
      con.query(`SELECT * FROM produk`, (err, res) => {
        if (err) callback(err, null);
        else callback(null, res);
      });
  },
